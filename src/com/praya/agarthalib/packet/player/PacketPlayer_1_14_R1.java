...
public class PacketPlayer_1_14_R1 extends HandlerPacket implements PacketPlayerProperties, PacketPlayerVisibility, PacketPlayerCombatPresent, PacketPlayerAbsorption
{
    public PacketPlayer_1_14_R1(final AgarthaLib plugin) {
        super(plugin);
    }
    ...
    public final float getSwingDuration(final Player player) {
        if (player != null) {
            final EntityPlayer entityPlayer = ((CraftPlayer)player).getHandle();
            final float duration = entityPlayer.dY();
            return duration;
        }
        return 0.0f;
    }
    ...
    public final void playSwingOffHand(final Player player) {
        ...
        if (player != null) {
            ...
            final EntityPlayer entityPlayer = ((CraftPlayer)player).getHandle();
            final PacketPlayOutAnimation packet = new PacketPlayOutAnimation((net.minecraft.server.v1_14_R1.Entity)entityPlayer, 3);
    ...
    public final void playSwingMainHand(final Player player) {
        ...
        if (player != null) {
            final Location location = player.getLocation();
            final EntityPlayer entityPlayer = ((CraftPlayer)player).getHandle();
            final PacketPlayOutAnimation packet = new PacketPlayOutAnimation((net.minecraft.server.v1_14_R1.Entity)entityPlayer, 0);
            final double effectRange = mainConfig.getEffectRange();
            for (final Player viewer : PlayerUtil.getNearbyPlayers(location, effectRange)) {
                final EntityPlayer entityViewer = ((CraftPlayer)viewer).getHandle();
                entityViewer.playerConnection.sendPacket((Packet)packet);
